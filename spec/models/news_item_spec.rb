...
RSpec.describe NewsItem, type: :model do
  ...
  describe 'querying News api' do
    it 'generates right query string' do
      ...
      expect(uri).to eq("https://newsapi.org/v2/everything?q=Joe Biden AND Abortion&from=#{Date.today - 30.days}&sort_by=relevancy&")
    end
    ...
    it 'returns right list of articles' do
      ...
      faraday_reponse = double('faraday reponse')
      allow(faraday_reponse).to receive(:body).and_return(article_json)
      expect(Faraday).to receive(:get).and_return(faraday_reponse)
